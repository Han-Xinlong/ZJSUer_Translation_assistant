...
表达改进库由 `errors` 状态驱动，存储在 `ERRORS_KEY`。
...
- 累计表达 = 完整历史记录数 + 表达库数量。
- 今日目标 = 今日历史记录 + 今日表达 + 今日改进记录。
- 待改进表达 = 表达改进库数量 + 当前润色修改数量 + 当前审校说明。
